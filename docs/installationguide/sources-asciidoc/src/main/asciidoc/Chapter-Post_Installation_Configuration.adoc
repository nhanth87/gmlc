...
These settings are specified in the file[path]_{this-folder}-gmlc-/wildfly-24.0.1.Final/bin/standalone.conf for Wildfly 24.
...
{this-platform} {this-application} can run only with Java 11 JRE or JDK.

=== Database Settings

Every transaction in the {this-platform} {this-application} is logged in a Database.
By default, transactions are logged in the HSQLDB that comes bundled with WildFly. However, this is only made available to allow the platform to run "out of the box". You must configure a production ready Database prior to using the Gateway in a production environment.
...
For WildFly 24.0.1.Final, datasource is configured in [path]_{this-folder}-gmlc-version>/wildfly-24.0.1.Final/standalone/configuration/standalone.xml file, "datasources" subsystem
...
{this-platform} {this-application} uses _Apache log4j_ for logging.
If you are not familiar with the _log4j_ package, you can read more about it at the Jakarta http://jakarta.apache.org/log4j/[website].

Logging is controlled from a central configuration file located at [path]_{this-folder}-gmlc/wildfly-24.0.1.Final/standalone/configuration/standalone.xml_.
...
For more details, please refer to _Logging Configuration_ section in the Wildfly documentation available  https://www.wildfly.org/guides/application-logging[here].

You must make sure log4j is fine-tuned for optimal performance in production. We recommend that you set logging threshold to `WARN` or `INFO` in a first phase, while letting the CDR appender be set to `DEBUG`.
...
The configuration at the _GmlcManagement_gmlcproperties.xml_ file should be consistent with MongoDB running host IP:port and database, for example:
[source,shell]
----
...
Configure the security limits by adding the soft and hard values to the GMLC user in the system's *limits.conf* file. For example
[source,shell]
----
...
Once the SS7 configuration is set up according to the data provided by the MNO, SS7 configuration files reside at $GMLC_HOME/wildfly-24.0.1.Final/standalone/data. Let's assume the configuration provided is as follows:
...
<diametergmlcnumber value="60132499947"/>
<diametergmlcaddress value="192.168.87.100"/>
<diametergmlcaddresstype value="4"/>
<riagmlcaddress value="false"/>
...
<supl_session_expired value="2880" />
<supl_periodic_session_expired value="7200" />
<redis_host value="localhost" />
<redis_port value="6379" />
<glaasenabled value="false"/>
<glaascdrendpoint value="http://localhost:9090/api/cdr" />
<glaasapptoken value="eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJnbWxjIiwiaWF0IjoxNjQ2MzIyOTc2fQ.Tnewo_3u2Zbe_wO7YAo7VJRXFEN7yCUgvJE8MiKU-kCClsakl5MZXHhlwst7JYBXtOkxobGbG-HRFs7ITDPqRw" />
<glassthreadpoll value="5"/>
<suplenabled value="true"/>
<smpphost value="localhost"/>
<smppport value="2776"/>
<smppsid value="gmlc_ulp_smpp"/>
<smpppwd value="gmlc_ulp_smpp"/>
<fqdn value="127.0.0.1"/>
----
...
Once the Diameter configuration is setup according to the data provided by the MNO, Diameter configuration files reside at $GMLC_HOME/wildfly-24.0.1.Final/standalone/data/*GmlcManagement_gmlcproperties.xml* and at $GMLC_HOME/wildfly-24.0.1.Final/modules/system/layers/base/org/restcomm/diameter/lib/main/config/*jdiameter-config.xml*.
