...
/**
 * NRTAIdImpl - NR (5G) Tracking Area Identity implementation
...
public class NRTAIdImpl extends OctetStringBase implements NRTAId {
    ...
    public NRTAIdImpl(byte[] data) {
        super(6, 6, "NRTAId", data);
    }
    ...
    public void setData(int mcc, int mnc, int tac) throws MAPException {
        ...
        // 5G TAC is 3 bytes (24 bits)
        data[3] = (byte) ((tac >> 16) & 0xFF);
    ...
    public int getMCC() throws MAPException {
        ...
        try {
            ...
            throw new MAPException("IOException when decoding NRTAId: " + e.getMessage(), e);
        } catch (MAPParsingComponentException e) {
            throw new MAPException("MAPParsingComponentException when decoding NRTAId: " + e.getMessage(), e);
        }
    ...
    public int getTAC() throws MAPException {
        ...
        return ((data[3] & 0xFF) << 16) + ((data[4] & 0xFF) << 8) + (data[5] & 0xFF);
    }
    ...
    protected static final XMLFormat<NRTAIdImpl> NR_TA_ID_XML = new XMLFormat<>(NRTAIdImpl.class) {
        ...
        @Override
        public void read(javolution.xml.XMLFormat.InputElement xml, NRTAIdImpl nrTaId) throws XMLStreamException {
            ...
            try {
                nrTaId.setData(mcc, mnc, tac);
            } catch (MAPException e) {
                throw new XMLStreamException("MAPException when deserializing NRTAIdImpl", e);
            }
        }

        @Override
        public void write(NRTAIdImpl nrTaId, javolution.xml.XMLFormat.OutputElement xml) throws XMLStreamException {
            try {
                xml.setAttribute(MCC, nrTaId.getMCC());
                xml.setAttribute(MNC, nrTaId.getMNC());
                xml.setAttribute(TAC, nrTaId.getTAC());
            } catch (MAPException e) {
                throw new XMLStreamException("MAPException when serializing NRTAIdImpl", e);
            }
        }
    };
}
